$(document).ready(function () {
  ...
  $(".version-selector .dropdown").hover(
    ...
    function() {
      ...
      if (isHidden) {
        ...
        $(this).find('.dropdown__button svg path').attr('fill', 'var(--version-unselected-color)');
      }
    }
  );
  ...
  $(document).on("click", function () {
    const dropdownList = $(
      ".toolbar:not(.hidden) .version-selector .dropdown__list"
    );
    const dropdownIcon = $(
      ".toolbar:not(.hidden) .version-selector .dropdown__button svg path"
    );
...
function updateDropdownIcon(dropdownList, dropdownIcon) {
  $(".version-selector .dropdown__container span").css({
    color: "var(--version-unselected-color)",
  });
  ...
  dropdownIcon.attr("fill", "var(--primary-dark)");
